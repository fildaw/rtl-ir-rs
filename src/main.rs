...
impl TryFrom<DeviceDescriptor> for Dongle {
    ...
    fn try_from(device_descriptor: DeviceDescriptor) -> Result<Self, Self::Error> {
        let dongle = KNOWN_DEVICES
            .iter()
            .find(|known_dongle| known_dongle.is_an_instance(&device_descriptor))
            .copied();
        dongle.ok_or(format!(
            "Device with vendor id {} and product id {} is not supported!",
...
fn open_device() -> Option<Device> {
    ...
    let found_device = devices.iter().find(|device| {
        KNOWN_DEVICES
            .iter()
            .any(|known_dongle| known_dongle.is_an_instance(&device.device_descriptor().unwrap()))
    });
